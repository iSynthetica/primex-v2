...
class snthNovaPoshtaApi2 {
	...
	function getWarehouse($cityRef, $description = '') {
		...
		$data = array();
		$type = $this->getWarehouseType($cityRef);
	 ...
	 * @return mixed
	 */
	function getWarehouseTypes($cityRef, $page = 0) {
		return $this->request('Address', 'getWarehouseTypes', array(
			'CityRef' => $cityRef,
			'Page' => $page,
		));
	}
	 ...
	 * @return mixed
	 */
	function getWarehouseType($cityRef, $description = '') {
		$warehouses = $this->getWarehouseTypes($cityRef);
		return $warehouses;
	}
	...
	function getArea($findByString = '', $ref = '') {
		...
		empty($this->areas) AND $this->areas = include dirname(__FILE__).'/class-nova-poshta-api2-areas.php';
	 ...
	 * @return mixed
	 */
	function getDocumentPrice($citySender, $cityRecipient, $serviceType, $weight, $cost, $volumegeneral) {
