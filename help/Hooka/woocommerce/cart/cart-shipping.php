...
$country_selected = $package['destination']['country'];
...
if ('UA' !== $country_selected) {
    $display = ' style="display:none"';
}
?>

<tr class="shipping"<?php echo $display ?>>
	<td data-title="<?php echo esc_attr( $package_name ); ?>" colspan="2">
		<?php if ( 1 < count( $available_methods ) ) : ?>

            <?php
            if ('UA' !== $country_selected) {
                $new_chosen_method = ('snth-npwd' === $chosen_method || 'snth-npww' === $chosen_method || 'snth-pickup' === $chosen_method) ? 'snth-other' : $chosen_method;

                if (!empty($available_methods['snth-npww'])) {
                    unset($available_methods['snth-npww']);
                }

                if (!empty($available_methods['snth-npwd'])) {
                    unset($available_methods['snth-npwd']);
                }

                if (!empty($available_methods['snth-pickup'])) {
                    unset($available_methods['snth-pickup']);
                }

                $chosen_method = $new_chosen_method;
            }
            ?>
            <p><strong><?php echo wp_kses_post( $package_name ); ?></strong></p>
